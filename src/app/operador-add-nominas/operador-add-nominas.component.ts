...
@Component({
  selector: 'app-operador-add-nominas',
  templateUrl: './operador-add-nominas.component.html',
  styleUrls: ['./operador-add-nominas.component.css']
})
export class OperadorAddNominasComponent implements OnInit {
  ...
  create_document(){
    ...
    this.crudService.add_document(this.document)
      .then(res => {
        ...
        this.newAction();
        console.log("id del doc creado " + res.data.idInserted)
        this.router.navigate(['./oper_nominas']);
      })
      .catch(err => {
        console.log(err);
      });
