...
@Component({
  selector: 'app-sup-add-rh',
  templateUrl: './sup-add-rh.component.html',
  styleUrls: ['./sup-add-rh.component.css']
})
export class SupAddRhComponent implements OnInit {
  ...
  create_document(){
    ...
    this.crudService.add_document(this.document)
      .then(res => {
        ...
        this.router.navigate(['./sup_recursos']);
      })
      .catch(err => {
        console.log(err);
      });
