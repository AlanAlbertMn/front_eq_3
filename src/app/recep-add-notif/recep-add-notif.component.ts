...
@Component({
  selector: 'app-recep-add-notif',
  templateUrl: './recep-add-notif.component.html',
  styleUrls: ['./recep-add-notif.component.css']
})
export class RecepAddNotifComponent implements OnInit {
  form: FormGroup;
  usertypes = [];
  notif = "";
  ...
  submit(){
    ...
    this.crudService.create_notification(this.not)
      .then(res => {
        ...
        this.router.navigate(['./rec_notif']);
      })
      .catch(err => {
        console.log(err);
      });
